...
{
    private void on_address_change(cef_display_handler_t* self, cef_browser_t* browser, cef_frame_t* frame,
        cef_string_t* url)
    {
        CheckSelf(self);
    ...
    private int on_console_message(cef_display_handler_t* self, cef_browser_t* browser, CefLogSeverity level,
        cef_string_t* message, cef_string_t* source, int line)
    {
        CheckSelf(self);
    ...
    /// </summary>
    protected virtual bool OnConsoleMessage(CefBrowser browser, CefLogSeverity level, string? message, string? source,
        int line)
    {
        return false;
    }
    ...
    private int on_cursor_change(cef_display_handler_t* self, cef_browser_t* browser, IntPtr cursor, CefCursorType type,
        cef_cursor_info_t* custom_cursor_info)
    {
        CheckSelf(self);
    ...
    /// </summary>
    protected virtual bool OnCursorChange(CefBrowser browser, IntPtr cursorHandle, CefCursorType type,
        CefCursorInfo? customCursorInfo)
    {
        return false;
    }

    private void on_media_access_change(cef_display_handler_t* self, cef_browser_t* browser, int has_video_access,
        int has_audio_access)
    {
        ...
        OnMediaAccessChange(m_browser, has_video_access != 0, has_audio_access != 0);
    }
