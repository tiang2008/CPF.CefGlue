...
public abstract unsafe partial class CefRequestHandler
{
    ...
    /// </summary>
    protected virtual bool OnBeforeBrowse(CefBrowser browser, CefFrame frame, CefRequest request, bool userGesture,
        bool isRedirect)
    {
        return false;
    }
    ...
    /// </summary>
    protected abstract CefResourceRequestHandler? GetResourceRequestHandler(CefBrowser? browser, CefFrame? frame,
        CefRequest request,
        bool isNavigation, bool isDownload, string? requestInitiator, ref bool disableDefaultHandling);


    private int get_auth_credentials(cef_request_handler_t* self, cef_browser_t* browser, cef_string_t* origin_url,
        int isProxy, cef_string_t* host, int port, cef_string_t* realm, cef_string_t* scheme,
        cef_auth_callback_t* callback)
    {
        ...
        var result = GetAuthCredentials(m_browser, m_originUrl, isProxy != 0, m_host, port, m_realm, m_scheme,
            m_callback);
    ...
    private int on_quota_request(cef_request_handler_t* self, cef_browser_t* browser, cef_string_t* origin_url,
        long new_size, cef_callback_t* callback)
    {
        CheckSelf(self);
    ...
    /// </summary>
    protected virtual bool OnCertificateError(CefBrowser browser, CefErrorCode certError, string? requestUrl,
        ...
        cef_select_client_certificate_callback_t* callback)
    {
        ...
        var m_host = cef_string_t.ToString(host);
        var m_certCount = (int) certificatesCount;
        var m_certificates = new CefX509Certificate[m_certCount];
        for (var i = 0; i < m_certCount; i++) m_certificates[i] = CefX509Certificate.FromNative(certificates[i]);
    ...
    /// </summary>
    protected virtual bool OnSelectClientCertificate(CefBrowser browser, bool isProxy, string? host, int port,
