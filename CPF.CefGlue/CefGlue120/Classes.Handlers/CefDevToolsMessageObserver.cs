...
{
    private int on_dev_tools_message(cef_dev_tools_message_observer_t* self, cef_browser_t* browser, void* message,
        UIntPtr message_size)
    {
        ...
        var m_result = OnDevToolsMessage(m_browser, (IntPtr) message, (int) message_size);
    ...
    private void on_dev_tools_method_result(cef_dev_tools_message_observer_t* self, cef_browser_t* browser,
        int message_id, int success, void* result, UIntPtr result_size)
    {
        CheckSelf(self);

        var m_browser = CefBrowser.FromNative(browser);

        OnDevToolsMethodResult(m_browser, message_id, success != 0, (IntPtr) result, (int) result_size);
    }
    ...
    /// </summary>
    protected abstract void OnDevToolsMethodResult(CefBrowser browser, int messageId, bool success, IntPtr result,
        int resultSize);


    private void on_dev_tools_event(cef_dev_tools_message_observer_t* self, cef_browser_t* browser,
        cef_string_t* method, void* @params, UIntPtr params_size)
    {
        CheckSelf(self);
        ...
        OnDevToolsEvent(m_browser, m_method, (IntPtr) @params, (int) params_size);
    }
