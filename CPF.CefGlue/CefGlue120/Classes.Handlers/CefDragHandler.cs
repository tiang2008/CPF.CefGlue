...
public abstract unsafe partial class CefDragHandler
{
    ...
    private int on_drag_enter(cef_drag_handler_t* self, cef_browser_t* browser, cef_drag_data_t* dragData,
    ...
    private void on_draggable_regions_changed(cef_drag_handler_t* self, cef_browser_t* browser, cef_frame_t* frame,
        UIntPtr regionsCount, cef_draggable_region_t* regions)
    {
        CheckSelf(self);
        ...
        else
        {
            ...
            for (var i = 0; i < m_count; i++) m_regions[i] = CefDraggableRegion.FromNative(regions + i);
        }
