...
public abstract unsafe partial class CefMediaObserver
{
    private void on_sinks(cef_media_observer_t* self, UIntPtr sinksCount, cef_media_sink_t** sinks)
    {
        ...
        var mSinksCount = (int) sinksCount;
        var mSinks = new CefMediaSink[mSinksCount];
        for (var i = 0; i < mSinksCount; i++) mSinks[i] = CefMediaSink.FromNative(sinks[i]);
    ...
    private void on_routes(cef_media_observer_t* self, UIntPtr routesCount, cef_media_route_t** routes)
    {
        ...
        var mRoutesCount = (int) routesCount;
        var mRoutes = new CefMediaRoute[mRoutesCount];
        for (var i = 0; i < mRoutesCount; i++) mRoutes[i] = CefMediaRoute.FromNative(routes[i]);
    ...
    private void on_route_state_changed(cef_media_observer_t* self, cef_media_route_t* route,
    ...
    private void on_route_message_received(cef_media_observer_t* self, cef_media_route_t* route, void* message,
        UIntPtr message_size)
    {
        CheckSelf(self);

        var mRoute = CefMediaRoute.FromNative(route);
        var mMessageSize = (int) message_size;
