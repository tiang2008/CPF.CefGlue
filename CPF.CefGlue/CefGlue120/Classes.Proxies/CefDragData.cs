...
public sealed unsafe partial class CefDragData
{
    ...
    /// </summary>
    public bool IsReadOnly => cef_drag_data_t.is_read_only(_self) != 0;


    /// <summary>
    ///     Returns true if the drag data is a link.
    /// </summary>
    public bool IsLink => cef_drag_data_t.is_link(_self) != 0;

    /// <summary>
    ///     Returns true if the drag data is a text or html fragment.
    /// </summary>
    public bool IsFragment => cef_drag_data_t.is_fragment(_self) != 0;

    /// <summary>
    ///     Returns true if the drag data is a file.
    /// </summary>
    public bool IsFile => cef_drag_data_t.is_file(_self) != 0;
    ...
    /// </summary>
    public bool HasImage => cef_drag_data_t.has_image(_self) != 0;
    ...
    {
        return FromNative(cef_drag_data_t.create());
    }
    ...
    {
        return FromNative(cef_drag_data_t.clone(_self));
    }
    ...
    public void SetLinkTitle(string? title)
    {
        ...
        {
            var n_title = new cef_string_t(title_str, title?.Length ?? 0);
    ...
    public void SetFragmentHtml(string? html)
    {
        ...
        {
            var n_html = new cef_string_t(html_str, html?.Length ?? 0);
    ...
    public void SetFragmentBaseURL(string? baseUrl)
    {
        ...
        {
            var n_baseUrl = new cef_string_t(baseUrl_str, baseUrl?.Length ?? 0);
    ...
    public void AddFile(string? path, string? displayName)
    {
        ...
        {
            var n_path = new cef_string_t(path_str, path?.Length ?? 0);
            var n_displayName = new cef_string_t(displayName_str, displayName?.Length ?? 0);
