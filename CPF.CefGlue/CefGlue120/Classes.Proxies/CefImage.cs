...
public sealed unsafe partial class CefImage
{
    ...
    /// </summary>
    public bool IsEmpty => cef_image_t.is_empty(_self) != 0;
    ...
    /// </summary>
    public bool AddBitmap(float scaleFactor, int pixelWidth, int pixelHeight, CefColorType colorType,
        CefAlphaType alphaType, IntPtr pixelData, int pixelDataSize)
    {
        ...
        var n_result = cef_image_t.add_bitmap(_self, scaleFactor, pixelWidth, pixelHeight, colorType, alphaType,
            (void*) pixelData, (UIntPtr) pixelDataSize);
        return n_result != 0;
    }
    ...
    /// </summary>
    public bool GetRepresentationInfo(float scaleFactor, out float actualScaleFactor, out int pixelWidth,
        out int pixelHeight)
    {
        ...
        int n_pixelHeight;
        var n_result =
            cef_image_t.get_representation_info(_self, scaleFactor, &n_actualScaleFactor, &n_pixelWidth,
                &n_pixelHeight);
        if (n_result != 0)
        {
    ...
    ///     failure.
    /// </summary>
    public CefBinaryValue? GetAsBitmap(float scaleFactor, CefColorType colorType, CefAlphaType alphaType,
        out int pixelWidth, out int pixelHeight)
    {
        int n_pixelWidth;
        int n_pixelHeight;
        var n_result =
            cef_image_t.get_as_bitmap(_self, scaleFactor, colorType, alphaType, &n_pixelWidth, &n_pixelHeight);
        if (n_result != null)
        {
            pixelWidth = n_pixelWidth;
            pixelHeight = n_pixelHeight;
    ...
    public CefBinaryValue? GetAsPng(float scaleFactor, bool withTransparency, out int pixelWidth, out int pixelHeight)
    {
        ...
        int n_pixelHeight;
        var n_result =
            cef_image_t.get_as_png(_self, scaleFactor, withTransparency ? 1 : 0, &n_pixelWidth, &n_pixelHeight);
        if (n_result != null)
        {
            pixelWidth = n_pixelWidth;
            pixelHeight = n_pixelHeight;
