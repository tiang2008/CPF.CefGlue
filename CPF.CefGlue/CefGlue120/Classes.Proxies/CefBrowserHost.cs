...
public sealed unsafe partial class CefBrowserHost
{
        ...
        CefDictionaryValue? extraInfo = null, CefRequestContext? requestContext = null)
    {
        ...
        {
            var n_url = new cef_string_t(url_ptr, url?.Length ?? 0);
            var n_success = cef_browser_host_t.create_browser(n_windowInfo, n_client, &n_url, n_settings, n_extraInfo,
                n_requestContext);
    ...
    ///     thread.
    /// </summary>
    public void RunFileDialog(CefFileDialogMode mode, string? title, string? defaultFilePath, string[] acceptFilters, 
    ...
    /// </summary>
    public void DownloadImage(string imageUrl, bool isFavIcon, uint maxImageSize, bool bypassCache,
        CefDownloadImageCallback callback)
    {
        ...
        fixed (char* imageUrl_ptr = imageUrl)
        {
            ...
            cef_browser_host_t.download_image(_self, &n_imageUrl, isFavIcon ? 1 : 0, maxImageSize, bypassCache ? 1 : 0,
                n_callback);
        }
    }
    ...
    {
        @event.ToNative(out var n_event);
        ...
        CefRange selectionRange)
    {
        ...
        {
            var n_text = new cef_string_t(text_ptr, text?.Length ?? 0);
            var n_underlinesCount = (UIntPtr) underlinesCount;
