...
public sealed unsafe partial class CefServer
{
    ...
    ///     server thread.
    /// </summary>
    public bool IsRunning => cef_server_t.is_running(_self) != 0;

    /// <summary>
    ///     Returns the server address including the port number.
    /// </summary>
    public string Address =>
        cef_string_userfree.ToString(
    ...
    /// </summary>
    public bool HasConnection => cef_server_t.has_connection(_self) != 0;
    ...
    public void SendHttp200Response(int connectionId, string contentType, IntPtr data, long dataSize)
    {
        fixed (char* contentType_str = contentType)
        {
            ...
            var n_dataSize = (UIntPtr) dataSize;
    ...
    /// </summary>
    public void SendHttpResponse(int connectionId, int responseCode, string contentType, long contentLength,
        NameValueCollection extraHeaders)
    {
        fixed (char* contentType_str = contentType)
        {
            ...
            cef_server_t.send_http_response(_self, connectionId, responseCode, &n_contentType, contentLength,
                n_extraHeaders);
