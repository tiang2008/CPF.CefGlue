...
internal static unsafe partial class libcef
{
    ...
    {
        return (int) string_multimap_size_core(map);
    }
    ...
    {
        return (int) string_multimap_find_count_core(map, key);
    }

    // Return the value_index-th value with the specified key.
    [DllImport(DllName, EntryPoint = "cef_string_multimap_enumerate", CallingConvention = CEF_CALL)]
    private static extern int string_multimap_enumerate_core(cef_string_multimap* map, cef_string_t* key,
        UIntPtr value_index, cef_string_t* value);

    public static int string_multimap_enumerate(cef_string_multimap* map, cef_string_t* key, int value_index,
        cef_string_t* value)
    {
        return string_multimap_enumerate_core(map, key, (UIntPtr) value_index, value);
    }
    ...
    {
        return string_multimap_key_core(map, (UIntPtr) index, key);
    }
    ...
    {
        return string_multimap_value_core(map, (UIntPtr) index, value);
    }
