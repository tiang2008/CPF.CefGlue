...
namespace CPF.CefGlue.JSExtenstions
{
    internal static class JavascriptToNativeTypeConverter
    {
        ...
        private static bool IsCollection(this Type source)
        {
            ...
            return source.IsGenericType && source.GetInterfaces().Any(i => i.IsGenericType && i.GetGenericTypeDefinition() == collectionType);
        }
        ...
        {
            return source.IsGenericType && source.GetGenericTypeDefinition() == typeof(IEnumerable<>);
        }
        ...
        private static ListContructor InnerGetListConstructor(Type expectedType)
        {
            ...
            {
                listType = expectedType.GetInterfaces().Where(itf => itf.IsGenericType).FirstOrDefault(itf => itf.GetGenericTypeDefinition() == typeof(IEnumerable<>));
            }
        ...
        private static IEnumerable<MemberInfo> InnerCollectMembers(Type type)
        {
            ...
            var properties = type
                ...
                .Select(p => new MemberInfo(p.Name, p.PropertyType, (obj, value) => p.FastSetValue(obj, value)));
