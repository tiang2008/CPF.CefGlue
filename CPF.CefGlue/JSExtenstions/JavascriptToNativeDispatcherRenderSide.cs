...
namespace CPF.CefGlue.JSExtenstions
{
    internal class JavascriptToNativeDispatcherRenderSide : INativeObjectRegistry
    {
        ...
        private void HandleNativeObjectCallResult(MessageReceivedEventArgs args)
        {
            ...
            {
                using (promiseHolder)
                using (promiseHolder.Context.EnterOrFail(shallDispose: false))
                {
            ...
            if (context.Enter())
            {
                try
                {
                    ...
                    foreach (var objectInfo in objectInfos)
                    {
                        ...
                        global.SetValue(objectInfo.Name, v8Obj, CefV8PropertyAttribute.None);
                    }
        ...
        {
#if NET40
            return _pendingBoundQueryTasks.GetOrAdd(objName, _ => new TaskCompletionSource<bool>()).Task;//TaskCreationOptions.RunContinuationsAsynchronously
#else
